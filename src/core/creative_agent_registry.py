...
class CreativeAgentRegistry:
    ...
    async def _fetch_for_agent_with_cache(
        self,
        agent: CreativeAgent,
        client: ADCPMultiAgentClient | None = None,
        ...
        """Fetch formats from one agent, with cache and stale-on-error fallback.
        ...
        - On fetch failure: if a cached entry exists and no filters are active, fall back
          to the cached formats and mark `stale=True`. With no usable cache, re-raise.
        """
        has_filters = any(
            v is not None
            for v in (
    ...
    async def list_all_formats_with_errors(
        ...
        for agent in agents:
            ...
            try:
                fetch = await self._fetch_for_agent_with_cache(
                    agent,
                    client=client,
                    force_refresh=force_refresh,
                    max_width=max_width,
