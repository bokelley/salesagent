...
def resolve_composed_products(tenant_id: str, product_ids: list[str]) -> list:
    """Look up storefront-composed products referenced in ``create_media_buy``.

    Composed products are excluded from the catalog (so they don't surface
    in ``get_products``) but are still valid targets for ``create_media_buy``
    when the storefront hands the buyer the materialized id. This helper
    fills that gap: it resolves each id, checks ``expires_at``, and raises
    :class:`AdCPProductNotFoundError` for anything unknown or expired.

    Args:
        tenant_id: Tenant scope.
        product_ids: ids that ``get_product_catalog`` did NOT include.

    Returns:
        List of ``ResolvedProduct`` for composed products in the input.
        Raises ``AdCPProductNotFoundError`` if any id is unknown or refers
        to a composed product whose ``expires_at`` is in the past.
    """
    ...
    with ProductUoW(tenant_id) as uow:
        assert uow.products is not None
        rows = uow.products.list_by_ids(product_ids)
        ...
        if missing:
            raise AdCPProductNotFoundError(
                f"Product(s) not found: {', '.join(sorted(missing))}",
            )
        ...
        eligible = []
        for row in rows:
            # Only composed/variant rows have meaningful expiry; static
            # rows that landed here are simply non-catalog (filtered out
            # of get_products for some other reason) and pass through.
            if row.composition_source != "static" and row.expires_at is not None:
                if row.expires_at < now:
                    expired.append(row.product_id)
                    continue
            eligible.append(row)

        if expired:
            raise AdCPProductNotFoundError(
                f"Composed product(s) expired: {', '.join(sorted(expired))}. "
                "Re-compose via POST /api/v1/products and retry with the new product_id.",
            )

        return [convert_product_model_to_resolved(row) for row in eligible]
